...
void displayBattle(/*const*/ Player* player, const std::vector<std::unique_ptr<Monster>>& monsters); // if const, will give you this error: stackoverflow.com/questions/20252932/the-object-has-type-qualifiers-that-are-not-compatible-with-the-member-function
...
int main()
{
	...
	while (!player->isDead())
	{
		...
		while (!player->isDead() && monsters.size() > 0)
		{
			...
			std::cout << std::endl;
			std::for_each(monsters.begin(), monsters.end(), [&](std::unique_ptr<Monster>& monster) // unique_ptr used to be Monster&
				{
...
std::vector<std::unique_ptr<Monster>> createMonsters(const Player* player)
{
	...
	/*std::normal_distribution<double> randomNumMonsters(5 / 2.0);*/
